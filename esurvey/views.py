...
def getPad(request,group_id):
    ...
    if 'joined' in request.session.keys():
        ...
        return render(request,'pad_only_no_aud.html',{'group':group_id,'session_obj':session_obj.session,'session':request.session['joined'],'form':form,'etherpad_url':settings.ETHERPAD_URL,'padname':pad.eth_padid,'sessionid':eth_session})
    else:
...
def getSession(request,session_id):
    ...
    if session.count() == 0:
        ...
        return render(request,'session_main_no_aud.html',{'session':session,'eth_group':eth_group,'no_group':list(range(session.groups))})
